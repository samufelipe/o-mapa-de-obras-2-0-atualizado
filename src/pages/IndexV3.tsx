...
const IndexV3 = () => {
  ...
  const scrollToForm = () => {
    trackCTAClick("sticky_cta", "mobile_bottom_v3", "QUERO MINHA VAGA AGORA");
    handleCTA();
  };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className={`fixed bottom-0 left-0 w-full z-[100] md:hidden transition-all duration-500 transform bg-background/95 backdrop-blur-sm border-t border-border px-4 py-3 ${showStickyCTA ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0"}`}>
        <button
          ...
          className="w-full bg-green-600 text-white py-4 px-4 text-sm font-black uppercase tracking-wide shadow-2xl border-2 border-green-600 flex items-center justify-between group active:scale-95 hover:bg-green-700 transition-colors duration-300"
        >
