    ...
    <section id="bonus" className="py-20 bg-background border-t border-border reveal">
      <div className="container mx-auto px-4 max-w-5xl text-center">
          ...
          {BONUSES.map((bonus, idx) => {
            ...
            return (
              <div key={idx} className={`bg-card border-2 p-8 flex flex-col items-center group relative shadow-premium hover:shadow-premium-gold hover:-translate-y-1 transition-all duration-300 ${isImmediate ? 'border-primary' : 'border-foreground'}`}>
                {isImmediate && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-foreground px-3 py-1 text-[8px] font-bold uppercase tracking-widest whitespace-nowrap">
                    LIBERADO NA HORA
                  </div>
                )}
                ...
                <h3 className="text-[10px] font-bold uppercase mb-1 tracking-widest text-center">{bonus.title}</h3>
                <span className="text-[11px] font-bold bg-primary/10 px-3 py-1 uppercase tracking-tighter mb-4">
                  Valor: {bonus.value}
                </span>
                ...
                <div className="mt-auto pt-4 border-t border-border w-full">
                  <span className={`text-[8px] font-bold uppercase tracking-widest flex items-center justify-center gap-1 ${isImmediate ? 'text-primary' : 'text-muted-foreground'}`}>
        ...
        <div className="max-w-xl mx-auto bg-foreground border-2 border-primary p-8 md:p-12 shadow-premium relative overflow-hidden">
          <div className="absolute top-0 right-0 bg-primary text-foreground px-4 py-1 text-[9px] font-bold uppercase tracking-widest transform rotate-45 translate-x-8 translate-y-4">
            OFERTA ATIVA
          </div>
          ...
          <div className="mb-8 text-center">
            ...
            <div className="mb-2">
              <span className="text-primary text-[10px] font-bold uppercase tracking-[0.3em] block mb-1">INVESTIMENTO ÚNICO</span>
          ...
          <button 
            ...
            className="w-full bg-primary text-foreground py-5 text-xs font-bold uppercase tracking-[0.2em] hover:bg-background transition-all duration-300 shadow-premium hover:shadow-premium-gold hover:-translate-y-1 active:scale-95 group flex items-center justify-center gap-2"
          >
